...
import com.laya.word2vec.thrift.Word2VecModelThrift;
import com.laya.word2vec.util.AC;
import com.laya.word2vec.util.Common;
import com.laya.word2vec.util.ProfilingTimer;
...
public class SerializableWord2Vec implements Serializable {

    List<String> vocab;
    int layerSize;
    transient DoubleBuffer vectors;
    public Map<String, ImmutableList<Double>> rawVectors = new HashMap <String, ImmutableList<Double>>() ;
    private final static long ONE_GB = 1024 * 1024 * 1024;

    SerializableWord2Vec(Iterable<String> vocab, int layerSize, DoubleBuffer vectors) {
        Searcher searcher = new Word2VecModel(vocab, layerSize, vectors ).forSearch();
        this.vocab = ImmutableList.copyOf(vocab);
        this.layerSize = layerSize;
        this.vectors = vectors;
        for (String word : vocab){
            try {
                rawVectors.put(word, searcher.getRawVector(word));
    ...
    SerializableWord2Vec(Iterable<String> vocab, int layerSize, double[] vectors) {
        this(vocab, layerSize, DoubleBuffer.wrap(vectors));
    }

    public SerializableWord2Vec(Word2VecModel model){
        this(model.vocab, model.layerSize, model.vectors);
    }

    public SerializableWord2Vec() {
    ...
    public Searcher forSearch() {
        return new SerializableSearcher(this);
    }
